    ...
    def get(self, request):
        ...
        if context is None:
            ...
            for type in types:
                ...
                # 动态给type对象添加两个属性保存数据****和java不一样,这里可以直接添加属性
...
class DetailView(View):
    ...
    def get(self, request, goods_id):
        ...
        # 获取新品推荐信息  取得两个 这里使用create_time默认升序加上-降序
        new_skus = GoodsSKU.objects.filter(type=sku.type).order_by('-create_time')[:2]

        # 获取同一个SPU的其他规格商品,不包括它自己
        ...
        if user.is_authenticated:
            ...
            # 移除列表中的goods_id
            # ****注意这里 移除goods_id相同的id 第二个值是0 代表全部移除
...
class ListView(View):
    ...
    def get(self, request, type_id, page):
        ...
        # 获取第page页的Page实例对象 这里skus_page已经是具有分页功能的类
